...
int main(int argc, char* argv[]) {
    ...
    if (gridShader.isValid() == 0) {
		...
		Shader shadelessShader("shadeless_shader.vert", "shadeless_shader.frag");
		if (shadelessShader.isValid() == 0) {
			std::cout << "Failed to load shadeless_shader." << std::endl;
			return -1;
		}
...
//    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    ...
    ModelLoader model;
    model.LoadModel("res/models/pistol/test-mesh.gltf");
